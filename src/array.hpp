...
template <typename T, u16 size> struct StaticArray {
    union {
        T *Data;
        u8 *_data;
    };
...
template <typename T> struct Queue_Array : public Array<T> {
    Queue_Array() : Array<T>() {}
...
template <typename T> void test(const T &a, const T &b) {
    if (a == b) {
        printf("Passed\n");
    } else {
        printf("Failed\n");
...
void testArray() {
    ...
    printf("Reserve size = 150 ...");
    test<i32>(150, intArray.Reserve);
    printf("size = 98 ...");
    test<i32>(98, intArray.Size);
    printf("intArray[0] = 65 ...");
    test<i32>(65, intArray[0]);
    printf("intArray[40] = 3980 ...");
    test<i32>(3980, intArray[39]);
    printf("intArray[30] = 54023 ...");
    test<i32>(54023, intArray[29]);
    printf("intArray[size-1] = 98 ...");
    test<i32>(98, intArray[intArray.size - 1]);
    ...
    printf("Reserve size = 150 ...");
    test<i32>(150, fooArray.Reserve);
    printf("size = 98 ...");
    test<i32>(98, fooArray.Size);
    printf("fooArray[0].x = 13 ...");
    test<i32>(13, fooArray[0].x);
    printf("fooArray[0].y = 1.0 ...");
    test<r32>(1.0f, fooArray[0].y);
    printf("fooArray[39].x = 593 ...");
    test<i32>(593, fooArray[39].x);
    printf("fooArray[39].x = 124.04726 ...");
    test<r32>(124.04726f, fooArray[39].y);

    printf("fooArray[29].x = 4321 ...");
    test<i32>(4321, fooArray[29].x);
    printf("fooArray[29].y = %f ...", PI);
    test<r32>(PI, fooArray[29].y);

    printf("fooArray[size-1].x = 13 ...");
    test<i32>(13, fooArray[fooArray.Size - 1].x);
    printf("fooArray[size-1].y = 1.0 ...");
    test<i32>(1.0f, fooArray[fooArray.Size - 1].y);
}
#endif
} // namespace SMOBA
