...
Texture2D::Texture2D(i32 width, i32 height) : Width(width), Height(height) {
    glGenTextures(1, &TextureID);
    glBindTexture(GL_TEXTURE_2D, TextureID);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
...
void Renderer::Render(std::queue<RenderCommand> *renderQueue, Camera *camera) {
    ...
    for (i32 i = 0; i < commandCount; i++) {
        ...
        case TEXTURERENDER:
            Draw_Texture(ASSETS::Get_Texture(rc.Texture), camera[0],
                         rc.TextureRect, vec2(rc.Pos.x, rc.Pos.y),
                         vec2(rc.Scale.x, rc.Scale.y), rc.Rot, rc.Pos.z,
                         rc.Color, rc.ShaderType);
            break;
        case RECTANGLERENDER:
            Draw_Texture(0, camera[0], rc.TextureRect, vec2(rc.Pos.x, rc.Pos.y),
                         vec2(rc.Scale.x, rc.Scale.y), rc.Rot, rc.Pos.z,
                         rc.Color, rc.ShaderType);
            break;
        case STRINGRENDER:
            Render_String(rc.String, camera[0], vec2(rc.Pos.x, rc.Pos.y),
                          vec2(rc.Scale.x, rc.Scale.y), rc.Rot, rc.Color);
            break;
        case TEXTRECTRENDER:
            Draw_Text_Rect(rc.String, camera[0], rc.Pos.x, rc.Pos.y, rc.Scale.x,
                           rc.Scale.y, rc.Color);
            break;
        case MESHRENDER:
            ...
            Draw_Mesh(*ASSETS::Get_Mesh(rc.Mesh),
                      ASSETS::Get_Texture(rc.Texture), camera[1], rc.Pos,
                      rc.Scale, rc.Quat, rc.Color, rc.ShaderType);
...
void Renderer::Draw_Text_Rect(const char *str, Camera &camera, r32 x, r32 y,
                              r32 width, r32 height, vec4 color) {
    u32 strLength = strlen(str);
    u32 numCols = (u32)width / (RENDER_CHARACTER_WIDTH);
    s_assert(numCols <= RENDER_CHARACTER_MAX_PER_LINE,
    ...
    for (i32 l = 0; l < numLines; l++) {
        ...
        line[numCols] = 0;
        if (str[charIndex + numCols] != ' ' && strlen(line) == numCols) {
            line[endIndex] = 0;
        ...
        Render_String(line, camera, vec2(x, y - (RENDER_CHARACTER_HEIGHT * l)),
                      vec2(1.0f, 1.0f), 0.0f, color);
...
void Renderer::Render_String(const char *str, Camera &camera, vec2 pos,
                             vec2 scale, r32 rot, vec4 color) {
    i32 len = strlen(str);
    for (i32 i = 0; i < len; i++) {
        char current = str[i];
        i32 padding = 6;
        ...
        Draw_Texture(ASSETS::Get_Texture(ASSETS::TEXTURES::DebugBitMapFont),
                     camera, texRect, vec2(pos.x + ((i * (8))), pos.y),
                     vec2(scale.x, -scale.y), rot, 0.0f, color);
    }
}
...
void Renderer::Draw_Mesh(const Mesh &mesh, Texture2D *texture, Camera &camera,
    ...
    u32 materialAmbLoc =
        glGetUniformLocation(ShaderIDs[shader], "material.ambient");
    u32 materialDiffLoc =
        glGetUniformLocation(ShaderIDs[shader], "material.diffuse");
    u32 materialSpecLoc =
        glGetUniformLocation(ShaderIDs[shader], "material.specular");
    u32 materialShineLoc =
        glGetUniformLocation(ShaderIDs[shader], "material.shininess");
    u32 lightAmbLoc = glGetUniformLocation(ShaderIDs[shader], "light.ambient");
    u32 lightDiffLoc = glGetUniformLocation(ShaderIDs[shader], "light.diffuse");
    u32 lightSpecLoc =
        glGetUniformLocation(ShaderIDs[shader], "light.specular");
    u32 lightPosLoc = glGetUniformLocation(ShaderIDs[shader], "light.position");
    u32 viewPosLoc = glGetUniformLocation(ShaderIDs[shader], "viewPos");
    ...
    // u32 lightColorLoc = glGetUniformLocation(ShaderIDs[shader],
    // "lightColor"); u32 objectColorLoc =
    // glGetUniformLocation(ShaderIDs[shader], "objectColor");

    u32 mvpLoc = glGetUniformLocation(ShaderIDs[shader], "mvp");
...
void Renderer::Draw_3d_Debug_Line(vec3 point1, vec3 point2, vec4 color,
                                  Camera &camera) {

    r32 points[] = {point1.x, point1.y, point1.z, point2.x, point2.y, point2.z};
...
void Renderer::Draw_Texture(Texture2D *texture, Camera &camera,
                            iRect &textureRect, vec2 position, vec2 size,
                            r32 rotate, r32 depth, vec4 color,
                            ShaderType shader) {
    ...
    if (shader != FILLRECTANGLE) {
        ...
        r32 texture_coord_offset_width = texture_pixel_ratio_x * textureRect.w;
        r32 texture_coord_offset_height = texture_pixel_ratio_y * textureRect.h;
    ...
    model.make_transform(vec3(position.x, position.y, 0.0f), scale,
...
void Renderer::Init_Render_Data() {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, VBOs[0]);
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(r32),
                          (GLvoid *)0);
    glEnableVertexAttribArray(0);

    glBindBuffer(GL_ARRAY_BUFFER, VBOs[1]);
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(r32),
                          (GLvoid *)0);
    ...
    glBindVertexArray(OverlayVAO);
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(r32),
                          (GLvoid *)0);
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(r32),
                          (GLvoid *)(2 * sizeof(r32)));
    glEnableVertexAttribArray(1);
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, FrameBuffer);
    // attach texture
    // Texture2D* frameTex = new Texture2D(viewportInfo->ScreenWidth,
    // viewportInfo->ScreenHeight);
    glGenTextures(1, &FrameTexture);
    glBindTexture(GL_TEXTURE_2D, FrameTexture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, Viewport->ScreenWidth,
                 Viewport->ScreenHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
