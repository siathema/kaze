...
struct vec2 {
    ...
    inline explicit vec2(const r32 coordinate[2])
    ...
    inline r32 operator[](i32 i) const { return *(&x + 1); }

    inline r32 &operator[](i32 i) { return *(&x + i); }
    ...
    inline r32 length() const { return sqrt(x * x + y * y); }

    inline r32 length_squared() const { return x * x + y * y; }
    ...
    inline vec2 perpendicular(void) const { return vec2(-y, x); }
...
struct iRect {
    ...
    inline b8 operator==(const iRect &rRect) const {
        return (x == rRect.x) && (y == rRect.y) && (w == rRect.w) &&
               (h == rRect.h);
    }
...
struct vec3 {
    ...
    inline explicit vec3(const r32 coordinate[3])
    ...
    inline r32 length() const { return sqrt(x * x + y * y + z * z); }

    inline r32 length_squared() const { return x * x + y * y + z * z; }
    ...
    inline vec3 cross(const vec3 &rvector) const {
        return vec3(y * rvector.z - z * rvector.y,
...
struct vec4 {
    ...
    inline explicit vec4(r32 scalar)
        : x(scalar), y(scalar), z(scalar), w(scalar) {}

    inline explicit vec4(const r32 coordinate[4])
        : x(coordinate[0]), y(coordinate[1]), z(coordinate[2]),
          w(coordinate[3]) {}
    ...
    inline friend vec4 operator*(r32 scalar, const vec4 &rvector) {
        return vec4(scalar * rvector.x, scalar * rvector.y, scalar * rvector.z,
                    scalar * rvector.w);
    }
...
struct mat3 {
    ...
    inline mat3(const mat3 &other) { memcpy(m, other.m, 9 * sizeof(r32)); }

    mat3(r32 entry00, r32 entry01, r32 entry02, r32 entry10, r32 entry11,
         r32 entry12, r32 entry20, r32 entry21, r32 entry22) {
    ...
    inline const r32 *operator[](i32 row) const { return m[row]; }

    inline r32 *operator[](i32 row) { return m[row]; }
    ...
    inline b8 operator!=(const mat3 &other) const { return !operator==(other); }
...
struct quat {
    ...
    inline quat(const mat3 &rot) { this->from_rotation_matrix(rot); }
    ...
    inline quat(const vec3 axes[3]) { this->from_axes(axes); }

    inline quat(r32 *valptr) { memcpy(&w, valptr, sizeof(r32) * 4); }

    inline r32 operator[](i32 i) const { return *(&w + i); }

    inline r32 &operator[](i32 i) { return *(&w + i); }
    ...
    inline b8 operator==(const quat &rquat) const {
        return (rquat.x == x) && (rquat.y == y) && (rquat.z == z) &&
               (rquat.w == w);
    }

    inline b8 operator!=(const quat &rquat) const { return !operator==(rquat); }
    ...
    static quat slerp_extra_spins(r32 T, const quat &aquat, const quat &bquat,
                                  i32 extraSpins);

    // setup for spherical quadratic interpoliation
    static void intermediate(const quat &quat0, const quat &quat1,
                             const quat &quat2, quat &outa, quat &outb);

    // spherical quadratic interpolation
    static quat squad(r32 T, const quat &aquat, const quat &outa,
                      const quat &outb, const quat &bquat);

    static quat nlerp(r32 T, const quat &aquat, const quat &bquat,
                      b8 shortestPath = false);
};
...
struct mat4 {
    ...
    inline mat4(r32 m00, r32 m01, r32 m02, r32 m03, r32 m10, r32 m11, r32 m12,
                r32 m13, r32 m20, r32 m21, r32 m22, r32 m23, r32 m30, r32 m31,
                r32 m32, r32 m33) {
        m[0][0] = m00;
        m[0][1] = m01;
        m[0][2] = m02;
    ...
    inline r32 *operator[](i32 column) { return m[column]; }

    inline const r32 *operator[](i32 column) const { return m[column]; }

    inline mat4 concatenate(const mat4 &m2) const {
        mat4 r;
        r.m[0][0] = m[0][0] * m2[0][0] + m[0][1] * m2.m[1][0] +
                    m[0][2] * m2.m[2][0] + m[0][3] * m2.m[3][0];
        r.m[0][1] = m[0][0] * m2[0][1] + m[0][1] * m2.m[1][1] +
                    m[0][2] * m2.m[2][1] + m[0][3] * m2.m[3][1];
        r.m[0][2] = m[0][0] * m2[0][2] + m[0][1] * m2.m[1][2] +
                    m[0][2] * m2.m[2][2] + m[0][3] * m2.m[3][2];
        r.m[0][3] = m[0][0] * m2[0][3] + m[0][1] * m2.m[1][3] +
                    m[0][2] * m2.m[2][3] + m[0][3] * m2.m[3][3];

        r.m[1][0] = m[1][0] * m2[0][0] + m[1][1] * m2.m[1][0] +
                    m[1][2] * m2.m[2][0] + m[1][3] * m2.m[3][0];
        r.m[1][1] = m[1][0] * m2[0][1] + m[1][1] * m2.m[1][1] +
                    m[1][2] * m2.m[2][1] + m[1][3] * m2.m[3][1];
        r.m[1][2] = m[1][0] * m2[0][2] + m[1][1] * m2.m[1][2] +
                    m[1][2] * m2.m[2][2] + m[1][3] * m2.m[3][2];
        r.m[1][3] = m[1][0] * m2[0][3] + m[1][1] * m2.m[1][3] +
                    m[1][2] * m2.m[2][3] + m[1][3] * m2.m[3][3];

        r.m[2][0] = m[2][0] * m2[0][0] + m[2][1] * m2.m[1][0] +
                    m[2][2] * m2.m[2][0] + m[2][3] * m2.m[3][0];
        r.m[2][1] = m[2][0] * m2[0][1] + m[2][1] * m2.m[1][1] +
                    m[2][2] * m2.m[2][1] + m[2][3] * m2.m[3][1];
        r.m[2][2] = m[2][0] * m2[0][2] + m[2][1] * m2.m[1][2] +
                    m[2][2] * m2.m[2][2] + m[2][3] * m2.m[3][2];
        r.m[2][3] = m[2][0] * m2[0][3] + m[2][1] * m2.m[1][3] +
                    m[2][2] * m2.m[2][3] + m[2][3] * m2.m[3][3];

        r.m[3][0] = m[3][0] * m2[0][0] + m[3][1] * m2.m[1][0] +
                    m[3][2] * m2.m[2][0] + m[3][3] * m2.m[3][0];
        r.m[3][1] = m[3][0] * m2[0][1] + m[3][1] * m2.m[1][1] +
                    m[3][2] * m2.m[2][1] + m[3][3] * m2.m[3][1];
        r.m[3][2] = m[3][0] * m2[0][2] + m[3][1] * m2.m[1][2] +
                    m[3][2] * m2.m[2][2] + m[3][3] * m2.m[3][2];
        r.m[3][3] = m[3][0] * m2[0][3] + m[3][1] * m2.m[1][3] +
                    m[3][2] * m2.m[2][3] + m[3][3] * m2.m[3][3];

        return r;
    }

    inline mat4 operator*(const mat4 &m2) const { return concatenate(m2); }
    ...
    inline b8 operator==(const mat4 &m2) const {
        if (m[0][0] != m2.m[0][0] || m[0][1] != m2.m[0][1] ||
            m[0][2] != m2.m[0][2] || m[0][3] != m2.m[0][3] ||
            m[1][0] != m2.m[1][0] || m[1][1] != m2.m[1][1] ||
            m[1][2] != m2.m[1][2] || m[1][3] != m2.m[0][3] ||
            m[2][0] != m2.m[2][0] || m[2][1] != m2.m[2][1] ||
            m[2][2] != m2.m[2][2] || m[1][3] != m2.m[0][3] ||
            m[3][0] != m2.m[3][0] || m[3][1] != m2.m[3][1] ||
            m[3][2] != m2.m[3][2] || m[1][3] != m2.m[0][3])
            return false;
        return true;
    }
    ...
    inline mat4 transpose(void) const {
        return mat4(m[0][0], m[0][1], m[0][2], m[0][3], m[1][0], m[1][1],
    ...
    inline void rotation(const mat3 &r) { operator=(r); }
    ...
    inline mat4 operator*(r32 scalar) const {
        return mat4(scalar * m[0][0], scalar * m[0][1], scalar * m[0][2],
                    scalar * m[0][3], scalar * m[1][0], scalar * m[1][1],
                    scalar * m[1][2], scalar * m[1][3], scalar * m[2][0],
                    scalar * m[2][1], scalar * m[2][2], scalar * m[2][3],
                    scalar * m[3][0], scalar * m[3][1], scalar * m[3][2],
                    scalar * m[3][3]);
    }
    ...
    void make_transform(const vec3 &position, const vec3 &scale,
                        const quat &orientation);
    void make_inverse_transform(const vec3 &position, const vec3 &scale,
                                const quat &orientation);
    ...
    inline mat4 concatenate_affine(const mat4 &m2) const {
        return mat4(
            ...
            m[0][0] * m2.m[0][3] + m[0][1] * m2.m[1][3] + m[0][2] * m2.m[2][3] +
                m[0][3],

            m[1][0] * m2.m[0][0] + m[1][1] * m2.m[1][0] + m[1][2] * m2.m[2][0],
            m[1][0] * m2.m[0][1] + m[1][1] * m2.m[1][1] + m[1][2] * m2.m[2][1],
            m[1][0] * m2.m[0][2] + m[1][1] * m2.m[1][2] + m[1][2] * m2.m[2][2],
            m[1][0] * m2.m[0][3] + m[1][1] * m2.m[1][3] + m[1][2] * m2.m[2][3] +
                m[1][3],

            m[2][0] * m2.m[0][0] + m[2][1] * m2.m[1][0] + m[2][2] * m2.m[2][0],
            m[2][0] * m2.m[0][1] + m[2][1] * m2.m[1][1] + m[2][2] * m2.m[2][1],
            m[2][0] * m2.m[0][2] + m[2][1] * m2.m[1][2] + m[2][2] * m2.m[2][2],
            m[2][0] * m2.m[0][3] + m[2][1] * m2.m[1][3] + m[2][2] * m2.m[2][3] +
                m[2][3],
...
namespace sMath {
vec3 triangle_closest_point(const vec3 &, const vec3 &, const vec3 &,
                            const vec3 &);

inline r32 clampf(r32 t, r32 a, r32 b) { return f_min(b, f_max(a, t)); }

inline r32 lerpf(r32 t, r32 a, r32 b) { return a + (b - a) * t; }

inline r64 lerpd(r64 t, r64 a, r64 b) { return a + (b - a) * t; }
...
inline r32 angle_range(r32 x) { return closest_angle(x, 0.0f); }

inline r32 angle_to(r32 x, r32 y) { return y - closest_angle(x, y); }
...
enum class RaySphereIntersection : i8 { FrontFace, BackFace, count };

b8 ray_sphere_intersect(
    const vec3 &, const vec3 &, const vec3 &, vec3 * = 0,
    RaySphereIntersection = RaySphereIntersection::FrontFace);
