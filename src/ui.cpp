...
bool Is_Hot(UI_ID id) { return id == GlobalSync->UiContext->Hot; }

bool Is_Active(UI_ID id) { return id == GlobalSync->UiContext->Active; }
...
b8 UI_Mouse_Rect_Clicked(r32 x, r32 y, r32 width, r32 height) {
    ...
    if (mousePos.x > x && mousePos.x < x + width && mousePos.y > y &&
        mousePos.y < y + height && GlobalSync->Ip->LeftMouseDown) {
        return true;
    }
    return false;
}
...
bool UI_Do_Rectangle(UI_ID id, r32 x, r32 y, r32 width, r32 height,
                     const vec4 &color) {
...
bool UI_Do_Button(UI_ID id, r32 x, r32 y, r32 width, r32 height,
                  const char *label) {
    UI_State_Rect_Mouse(id, x, y - height, width, height);
...
bool UI_Do_Panel(UI_ID id, r32 x, r32 y, r32 width, r32 height,
                 const char *label) {
...
bool UI_Do_Text_Label(UI_ID id, r32 x, r32 y, r32 width, const char *label,
                      bool border) {
    bool result = true;
...
bool UI_Do_Text_Rect(UI_ID id, r32 x, r32 y, r32 width, r32 height,
                     const char *label, bool border) {
    bool result = true;
...
void Draw_Debug_Info_Panel(Sync *gameSync, Debug_Info_Panel *panel) {
    if (panel->Moving && gameSync->Ip->LeftMouseDown) {
        UI_Do_Panel(panel->Panel, panel->Pos.x, panel->Pos.y, panel->Size.x,
                    panel->Size.y, "Debug Info Panel");
    } else {
        panel->Moving =
            UI_Do_Panel(panel->Panel, panel->Pos.x, panel->Pos.y, panel->Size.x,
                        panel->Size.y, "Debug Info Panel");
    ...
    if (!panel->Closed) {
        UI_Do_Text_Rect(panel->TextRect, panel->Pos.x + 10, panel->Pos.y - 40,
                        panel->Size.x - 20, panel->Size.y - 200,
                        "this is a debug message. Boop");

        UI_Do_Text_Label(-1, panel->Pos.x + 10, panel->Pos.y - 240, 8 * 8,
                         "Value: ");

        i32 num = sprintf(panel->Message, "%f", panel->SliderValue);

        UI_Do_Text_Label(-1, panel->Pos.x + 10 + 56, panel->Pos.y - 240, 20 * 8,
                         panel->Message, true);

        if (panel->MovingSlider && gameSync->Ip->LeftMouseDown) {
            UI_Do_Slider(panel->Slider, panel->Pos.x + 10, panel->Pos.y - 200,
                         panel->Size.x - 20, panel->SliderValue);
        } else {
            panel->MovingSlider = UI_Do_Slider(
                panel->Slider, panel->Pos.x + 10, panel->Pos.y - 200,
                panel->Size.x - 20, panel->SliderValue);
        }
    }
}
...
void Do_Debug_Info_Panel(Sync *GameSync, Debug_Info_Panel *panel) {
    if (panel->Moving && GameSync->Ip->LeftMouseDown) {
        ...
        if (panel->Cooldown == 0) {
            if (UI_Mouse_Rect_Clicked(panel->Pos.x, panel->Pos.y - 20, 20.0f,
                                      20.0f)) {
        ...
        Draw_Debug_Info_Panel(GameSync, panel);
        panel->PreviousMouse = UI_Mouse_Convert_Coordinate(
            GameSync->Ip->MouseX, GameSync->Ip->MouseY);
    }
}
} // namespace SMOBA
